...
- Backend Vercel env vars: `ANTHROPIC_API_KEY`, `JWT_SECRET`, `DATABASE_URL`. Password lives only there; the repo is public, so never commit it.
...
- Entry: root `index.py` imports `main.app`. The preset routes requests as `/api/index/<path>`; `VercelPathFix` middleware in main.py strips it, so don't remove it. `GET /` is the health check (also touches DB so Supabase free tier doesn't pause).
